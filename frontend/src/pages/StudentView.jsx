...
const styles = `
  ...
  .custom-select { width: 100%; padding: 18px; border-radius: 15px; border: none; background: rgba(255, 255, 255, 0.95); font-size: 22px; color: #1756a9; margin-bottom: 20px; font-family: 'Rubik', sans-serif; font-weight: 600; direction: rtl; box-sizing: border-box; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
  
  .action-btn { width: 100%; background: linear-gradient(135deg, #0ea5e9, #2563eb); color: white; border: none; padding: 22px; border-radius: 20px; font-size: 28px; font-weight: 800; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 8px 25px rgba(37, 99, 235, 0.5); font-family: 'Rubik', sans-serif; }
  .action-btn:disabled { background: gray; box-shadow: none; cursor: not-allowed; opacity: 0.6; }
  .party-btn { width: 100%; background: white; color: #1756a9; border: none; padding: 24px; border-radius: 20px; font-size: 26px; font-weight: 800; cursor: pointer; margin-bottom: 18px; font-family: 'Rubik', sans-serif; box-shadow: 0 6px 20px rgba(0,0,0,0.15); }
  .summary-label { display: block; font-size: 22px; font-weight: bold; margin-bottom: 10px; text-align: right; }
`;
        ...
        {!isJoined ? (
          <div className="glass-card">
            ...
            <form onSubmit={handleJoin}>
              <input type="text" value={eventCode} onChange={(e) => setEventCode(e.target.value)} placeholder="קוד כניסה" className="custom-input" autoComplete="off" />
              ...
              <>
                {phase === 'warmup' && (
                  <div className="glass-card">
                    ...
                    {representatives.map(rep => ( <button key={rep} onClick={() => handleWarmupVote(rep)} className="party-btn">{rep}</button> ))}
                  </div>
                )}
                {phase.startsWith('round') && (
                  <div className="glass-card">
                    <div style={{ textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.25)', paddingBottom: '18px', marginBottom: '25px' }}>
                      <span style={{ background: 'rgba(255,255,255,0.25)', padding: '8px 20px', borderRadius: '20px', fontSize: '18px', fontWeight: 'bold' }}>סבב {phase.replace('round', '')}</span>
                    ...
                    {representatives.map(rep => ( <button key={rep} onClick={() => handleRoundVote(rep)} className="party-btn">{rep}</button> ))}
                  </div>
                )}
                {phase === 'summary' && (
                  <div className="glass-card" style={{ padding: '30px 20px' }}>
                    ...
                    <button onClick={handleSummarySubmit} className="action-btn" disabled={!isSummaryComplete} style={{ marginTop: '15px' }}>שלח שאלון</button>
